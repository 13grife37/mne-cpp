...
\begin{aims}
	\item[\hspace*{10mm} Qt Framework] Because MNE-CPP depends on the Qt framework, it must be installed. Therefore, you must download and install it.
\end{aims}

\begin{aims}
	\item[\hspace*{10mm} MNE-CPP Source Code] We recommend using Git to get the MNE-CPP source code, as you can simply clone / fork one of MNE-CPP's repositories.
\end{aims}

\begin{aims}
	\item[\hspace*{10mm} Compiling the Source Code] Since Qt creator is automatically installed when you install the Qt framework, it is easiest to use its build-in compiler. You must first run the \textit{mne-cpp.pro} file, then run \textit{qmake} and finally start a build of the project. After this, all binaries can be found inside the \textit{bin} folder.
\end{aims}
...
Thus, an MNEBemSurface must be obtained first: 
\begin{lstlisting}
...
The method \textit{GeometryInfo::projectSensors} returns a pointer to a vector of indices pointing to the best matching vertices of the passed mesh:
\begin{lstlisting}
...
The method \textit{GeometryInfo::scdc} receives three arguments: an MNEBemSurface that holds the mesh, a vector of indices (see Sensor Projecting for more details) and the distance threshold. The second argument represents the indices that distances should be calculated to (in most cases this is the result of the sensor mapping). An empty vector can be passed to indicate that all distances (all pairs) should be calculated. Duly note that this can consume a significantly larger amount of memory. The method outputs a pointer to a matrix that holds all calculated distance values.\\
\\
Since the third argument, i.e. the distance threshold is defaulted with infinity, it can be left out in case all distance values are wanted. Duly note that this may take a considerable amount of time.
...
\begin{lstlisting}
// scdc with distance threshold of 1.5 cm
QSharedPointer<MatrixXd> distanceTable;
distanceTable = GeometryInfo::scdc(surface, pVecMappedSubset, 0.015);
\end{lstlisting}
...
Assume that we have 49 vertices but only 2 sensors. The following figure illustrates how each vertex is influenced by the two sensors.
